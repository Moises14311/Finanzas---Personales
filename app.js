...
class FinancialCycle {
    ...
    startNewCycle(name, amount, notes = '') {
        const newCycle = {
    ...
    addExpense(amount, category, date, description = '', isRecurrent = false) {
        if (!this.currentCycle) return null;
        
        // Corregir problema de fecha (agregar un día)
        ...
        const expense = {
            ...
            category,
            date: correctedDate, // Usamos la fecha corregida
            description,
            isRecurrent,
    ...
    getPreviousCycle() {
        ...
        return this.cycles[this.cycles.length - 2];
    }

    compareWithPreviousCycle() {
...
class TripManager {
    ...
    startNewTrip(name, amount, startDate, notes = '') {
        const newTrip = {
            id: Date.now().toString(),
            name,
            initialAmount: parseFloat(amount),
    ...
    addTripExpense(amount, category, date, description = '') {
    ...
    getPreviousTrip() {
        if (this.trips.length < 2) return null;
        return this.trips[this.trips.length - 2];
    }

    compareWithPreviousTrip() {
...
const financialCycle = new FinancialCycle();
...
function loadPersonalFinanceData() {
...
function updateCurrentCycleInfo() {
    const currentCycle = financialCycle.currentCycle;
    const summary = financialCycle.getExpensesSummary();
...
function updateExpensesTable(filteredExpenses = null) {
    ...
    const expenses = filteredExpenses || 
        (financialCycle.currentCycle ? [...financialCycle.currentCycle.expenses].reverse() : []);
    ...
    document.querySelectorAll('#expensesTable .action-btn.delete').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (confirm('¿Estás seguro de eliminar este gasto?')) {
                if (financialCycle.deleteExpense(expenseId)) {
                    updatePersonalFinanceUI();
                }
            }
        });
...
function updateCyclesHistory() {
    ...
    if (financialCycle.cycles.length === 0) {
    ...
    const sortedCycles = [...financialCycle.cycles].sort((a, b) => 
...
function viewCycleDetails(cycleId) {
    const cycle = financialCycle.cycles.find(c => c.id === cycleId);
    if (!cycle) return;
...
function updateCategoryChart() {
    ...
    if (!financialCycle.currentCycle || financialCycle.currentCycle.expenses.length === 0) {
    ...
    const data = financialCycle.getExpensesByCategory();
...
function updateCycleComparison() {
    const comparison = financialCycle.compareWithPreviousCycle();
...
function handleNewCycleSubmit(e) {
    e.preventDefault();
    ...
    financialCycle.startNewCycle(name, amount, notes);
    newCycleModal.style.display = 'none';
    newCycleForm.reset();
...
function handleExpenseSubmit(e) {
    ...
    if (!financialCycle.currentCycle) {
    ...
    financialCycle.addExpense(amount, category, date, description, isRecurrent);
    expenseForm.reset();
...
function filterExpenses() {
    ...
    let filtered = financialCycle.currentCycle.expenses;
...
function updateCurrentTripInfo() {
    const currentTrip = tripManager.currentTrip;
    const summary = tripManager.getTripSummary();
...
function updateTripExpensesTable(filteredExpenses = null) {
    ...
    const expenses = filteredExpenses || 
        (tripManager.currentTrip ? [...tripManager.currentTrip.expenses].reverse() : []);
    ...
    document.querySelectorAll('#tripExpensesTable .action-btn.delete').forEach(btn => {
        btn.addEventListener('click', (e) => {
            const expenseId = e.currentTarget.dataset.id;
            if (confirm('¿Estás seguro de eliminar este gasto?')) {
                if (tripManager.deleteTripExpense(expenseId)) {
                    updateTripsUI();
                }
            }
        });
...
function updateTripsHistory() {
    ...
    const sortedTrips = [...tripManager.trips].sort((a, b) => 
        new Date(b.startDate) - new Date(a.startDate));
...
function viewTripDetails(tripId) {
    const trip = tripManager.trips.find(t => t.id === tripId);
    if (!trip) return;
...
function updateTripCategoryChart() {
    ...
    if (!tripManager.currentTrip || tripManager.currentTrip.expenses.length === 0) {
        document.getElementById('categoryChart').style.display = 'none';
        return;
    }
    ...
    const categoryMap = {};
    tripManager.currentTrip.expenses.forEach(expense => {
        if (!categoryMap[expense.category]) {
            categoryMap[expense.category] = 0;
        }
        categoryMap[expense.category] += expense.amount;
...
// Actualizar comparación con viaje anterior
function updateTripComparison() {
    const comparison = tripManager.compareWithPreviousTrip();
...
function handleNewTripSubmit(e) {
    ...
    tripManager.startNewTrip(name, amount, startDate, notes);
    newTripModal.style.display = 'none';
    newTripForm.reset();
...
function handleTripExpenseSubmit(e) {
    ...
    tripManager.addTripExpense(amount, category, date, description);
...
function filterTripExpenses() {
    ...
    let filtered = tripManager.currentTrip.expenses;
...
function exportPersonalData(type) {
    if (!financialCycle.currentCycle && financialCycle.cycles.length === 0) {
        alert('No hay datos para exportar');
        return;
    }
    
    const data = {
        currentCycle: financialCycle.currentCycle,
        cycles: financialCycle.cycles,
...
function exportTripData(type) {
    if (!tripManager.currentTrip && tripManager.trips.length === 0) {
        alert('No hay datos para exportar');
        return;
    }
